...
`publish-release-resumable.ps1` 用于已经完成 tag 推送和签名构建之后，将本地正式资产安全收敛到 GitHub Release。默认只读且不会修改公开清单；只有显式同时启用 `-Apply -PublishUpdaterManifest` 并通过全部清单和 `main` 防漂移门禁后，才会在同一提交中发布 `release/version.json` 与 `release/latest.json`。脚本不会读取 updater 私钥。
...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/publish-release-resumable.ps1 `
  ...
  -NotesFile release/RELEASE_NOTES-0.6.2.md `
  -PreflightOnly
```
...
- 资产目录必须且只能包含 `FanglvCaseBoard_<version>_x64-setup.exe` 及其同名 `.sig`，名称严格区分大小写且全 ASCII；
- 新 Release 先以 draft 创建，两个资产上传并逐项回读 SHA-256 后才转为正式 Release；中断重跑继续收敛同一 draft；
...
脚本会校验 draft 的版本、安装包 URL 和 signature 与正式 Release 资产完全一致，并以 Release 回读事实生成 `version.json`；随后原子替换两份清单，验证暂存区恰好只有这两个文件，再创建单一提交。推送前重新查询远端 `main`；仅允许从 `ExpectedMainCommit` 正常快进，网络超时后也会先查远端是否已经收敛。推送后还会从 raw GitHub 回读两份清单。任何远端漂移都会停止，不使用强推。
...
测试不访问网络，覆盖 EOF 恢复、超时耗尽、指数退避、精确 ASCII 资产对、中文/大小写/版本/数量变体失败关闭、正确资产跳过、缺失资产上传计划、本地/远端错误资产 fail closed、旧 API 无 digest 时回下载校验，以及双清单版本/URL/签名不一致、main 漂移和已推送收敛状态。
