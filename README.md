...
| 不是 | 由谁负责 |
|:---|:---|
| 写文书 / 合同审查 | [Qiuta.Law](https://qiuta.law) |
| AI 跑活 / 通用调研 | Claude Code / Codex / Hermes(后续通过 MCP 接入) |
| 法院短信 / 立案联动 | (V0.2 之后再说) |
...
| 模块 | 内容 |
|:---|:---|
| 📋 **诉讼** | 案件看板 · 9 档智能状态机(接案 / 立案中 / 待开庭 / 审理中 / 已调解 / 上诉期 / 二审 / 执行中 / 已结案) |
...
之后喂 DeepSeek 单次 LLM call → 输出**风险提示报告 MD**(参考资深律师风格:摘要 / 关键画像 / 被执行案件全景 / 失信限消 / 财产线索 / 核心差异与下一步行动)+ **深挖建议 JSON**(关联公司 / 新发现案号 / 第三方主体,V0.2 「🔬 深挖」按钮用)。
...
│   └── migrations/               # SQLite migrations(0001 ~ 0019)
