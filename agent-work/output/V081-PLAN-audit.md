...
| B7 | 版本号、安装包、签名和 updater 交付未完成 | 已关闭：签名 NSIS、updater manifest、Git tag、GitHub Release、公开回下载及正式升级均已通过 |
...
| Rust 全量 | 单元 259/259、设备同步 18/18，Windows CI 同步通过 | 通过 |
| CI | `30441264089`、`30441349767` | 通过 |
| 正式升级 | 0.8.0→0.8.1，迁移 54→59，`quick_check=ok`，业务表计数零变化 | 通过 |
| 发布 | `v0.8.1-fanglv`、四项公开资产、updater manifest、公开回下载 | 通过 |

本表仅转述已实际执行的结果。真实 NAS 双机和真实元典账号仍按外部环境待验，
不因安装包与发布完成而改写为已通过。
...
- 真实元典账号在线余额与官方页面核对；
- 设备同步在物理双机上的冲突、吊销、断网恢复与隔离恢复；
- 合法/恶意 Skills 文件在正式 GUI 中的导入、导出和回滚 smoke；
- 由用户继续进行日常业务路径的主观体验验收。

### 已完成的发布与正式升级

- 本地签名 NSIS 构建和 CI 独立 Windows 构建均通过；
- updater minisign 验证通过；Windows Authenticode 如实为 `NotSigned`；
- 正式库升级前有一致性备份，升级后迁移 54→59、`quick_check=ok`；
- 正式安装版本、卸载注册表均为 0.8.1，启动截图有效；
- `v0.8.1-fanglv` Release、四项资产和 `release/latest.json` 已公开；
- 公开安装包与签名已重新下载并复核 SHA-256 和 minisign。

## 六、最终判定

**v0.8.1 开发、自动化门禁、正式升级与公开发布均已完成。**

后续进入真实 NAS/物理双机和真实账号外部验收，不再登记为本版代码或发布缺口。
其中真实 NAS/物理双机验收必须等待用户完成 Windows 挂载；在该验收完成前，
只能表述为“设备同步代码和模拟双库契约已完成”，不能表述为“真实双机已验收”。

最终发布与正式升级证据见
`agent-work/output/V081-FINAL-20260729_发布与正式升级验收.md`。
