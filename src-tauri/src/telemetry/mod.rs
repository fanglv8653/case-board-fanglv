...
mod tests {
    ...
    #[test]
    fn disabled_under_test_build() {
        // cfg(test) 下必须恒为 false —— 防止 cargo test 联网上报。
        assert!(!enabled());
    }

    #[test]
    fn os_label_has_no_system_version() {
        let label = os_label();
        // 只含 "<os> <arch>"(如 "macos aarch64")。**不应带系统版本号**(如 15.1)——
        // 版本号带点号,而 os 名 / 架构名(aarch64/x86_64)都不带点,以此防回归。
        assert!(
            !label.contains('.'),
            "os_label 不应含系统版本号(带点): {label}"
        );
