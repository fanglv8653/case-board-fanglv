...
mod tests {
    ...
    #[test]
    fn is_safe_cache_relpath_rejects_traversal() {
        // 合法:普通相对路径(含子目录、./)
        assert!(is_safe_cache_relpath("SEARCH-abc.md"));
        assert!(is_safe_cache_relpath("sub/SEARCH-abc.md"));
        assert!(is_safe_cache_relpath("./SEARCH-abc.md"));
        // 非法:绝对路径 / 上跳 / 根 / 空
        assert!(!is_safe_cache_relpath("/etc/passwd"));
        assert!(!is_safe_cache_relpath("/Users/x/.zshrc"));
        assert!(!is_safe_cache_relpath("../../../../.zshrc"));
        assert!(!is_safe_cache_relpath("sub/../../escape.md"));
        assert!(!is_safe_cache_relpath(".."));
        assert!(!is_safe_cache_relpath(""));
        assert!(!is_safe_cache_relpath("   "));
    }

    /// 造一个 KB,可选注入若干缓存条目(以 (file_name, cached_at, query_type, summary, body) 形式)。
    fn make_kb(tmp: &TempDir, entries: &[(&str, &str, &str, &str, &str)]) -> LocalKb {
        ...
        for (i, (fname, cached_at, qtype, summary, body)) in entries.iter().enumerate() {
            std::fs::write(cache_dir.join(fname), body).unwrap();
            // 用 fname 当 hash key(测试场景下无所谓真假 hash;真实场景由 cache.rs 写)
            idx.insert(
                format!("key-{}", i),
                IndexEntry {
        ...
        // 用 auto_detect 走真实路径(造一个 Settings)
        let settings = Settings {
            local_kb_root: Some(root.to_string_lossy().to_string()),
            local_kb_enabled: Some(true),
            ..Default::default()
        };
        LocalKb::auto_detect(&settings).expect("auto_detect")
    }

    #[test]
    fn export_then_import_into_empty_kb_round_trip() {
        ...
        let out_zip = tmp_src.path().join("share.zip");
        let er = export_to_zip(
            &src_kb,
            ExportOptions {
                yuandian_cache_only: true,
                output_path: out_zip.clone(),
                include_readme: true,
                exporter_version: "test-0.0.1".to_string(),
            },
        )
        .unwrap();
        assert_eq!(er.total_items, 2);
        assert!(out_zip.exists());

        // 空 KB 导入
        let tmp_dst = TempDir::new().unwrap();
        let dst_kb = make_kb(&tmp_dst, &[]);
        let ir = import_from_zip(
            &dst_kb,
            ImportOptions {
                zip_path: out_zip,
        ...
        .unwrap();
        assert_eq!(ir.total_in_zip, 2);
        assert_eq!(ir.added, 2);
        assert_eq!(ir.skipped, 0);
        assert_eq!(ir.overwritten, 0);
        assert_eq!(ir.failed, 0);

        // 文件确实写到目的地
        assert!(dst_kb.yuandian_cache_dir.join("SEARCH-abc.md").exists());
        assert!(dst_kb
            .yuandian_cache_dir
            .join("法规-12345_民法典.md")
            .exists());

        // index.json 含 2 条
        let raw = std::fs::read_to_string(&dst_kb.index_path).unwrap();
        let parsed: HashMap<String, IndexEntry> = serde_json::from_str(&raw).unwrap();
        assert_eq!(parsed.len(), 2);
    }

    #[test]
    fn conflict_strategy_skip_keeps_existing() {
        let tmp_src = TempDir::new().unwrap();
        let src_kb = make_kb(
            &tmp_src,
            &[(
        ...
        let out_zip = tmp_src.path().join("s.zip");
        export_to_zip(
            &src_kb,
            ExportOptions {
                yuandian_cache_only: true,
        ...
        // dst 已有同 key 的 entry(注意 make_kb 是按数组下标生成 key,这里强制对齐)
        let tmp_dst = TempDir::new().unwrap();
        let dst_kb = make_kb(
            &tmp_dst,
            &[(
                "SEARCH-abc.md",
                "2026-05-27 09:00:00", // 旧
                "rh_ft_search",
                "existing summary",
                "existing body",
            )],
        );
        // src/dst 都用了 key-0,key 冲突 → Skip 应保留 existing
        let ir = import_from_zip(
            &dst_kb,
            ImportOptions {
                zip_path: out_zip,
        ...
        assert_eq!(ir.added, 0);
        assert_eq!(ir.overwritten, 0);

        let body =
            std::fs::read_to_string(dst_kb.yuandian_cache_dir.join("SEARCH-abc.md")).unwrap();
        assert_eq!(body, "existing body", "Skip 不应覆盖现有文件");

        let raw = std::fs::read_to_string(&dst_kb.index_path).unwrap();
        let parsed: HashMap<String, IndexEntry> = serde_json::from_str(&raw).unwrap();
        assert_eq!(parsed["key-0"].summary, "existing summary");
    }

    #[test]
    fn conflict_strategy_overwrite_older_only_overwrites_when_incoming_newer() {
        let tmp_src = TempDir::new().unwrap();
        let src_kb = make_kb(
            &tmp_src,
            &[(
                "SEARCH-abc.md",
                "2026-05-28 10:00:00", // incoming 较新
                "rh_ft_search",
                "incoming summary",
                "incoming body",
            )],
        ...
        let ir = import_from_zip(
            &dst_kb,
            ImportOptions {
                zip_path: out_zip.clone(),
                on_conflict: ConflictStrategy::OverwriteOlder,
            },
        )
        .unwrap();
        assert_eq!(ir.overwritten, 1);
        assert_eq!(ir.skipped, 0);
        let body =
            std::fs::read_to_string(dst_kb.yuandian_cache_dir.join("SEARCH-abc.md")).unwrap();
        assert_eq!(body, "incoming body");

        // 反过来:incoming 更旧 → Skip
        let tmp_dst2 = TempDir::new().unwrap();
        let dst_kb2 = make_kb(
            &tmp_dst2,
            &[(
                "SEARCH-abc.md",
                "2026-06-01 09:00:00", // dst 更新
                "rh_ft_search",
                "dst-new",
                "dst body new",
            )],
        );
        let ir2 = import_from_zip(
            &dst_kb2,
            ImportOptions {
                zip_path: out_zip,
                on_conflict: ConflictStrategy::OverwriteOlder,
        ...
        .unwrap();
        assert_eq!(ir2.skipped, 1);
        assert_eq!(ir2.overwritten, 0);
        let body2 =
            std::fs::read_to_string(dst_kb2.yuandian_cache_dir.join("SEARCH-abc.md")).unwrap();
        assert_eq!(body2, "dst body new", "OverwriteOlder 不应覆盖更新的文件");
    }

    #[test]
    fn conflict_strategy_always_overwrite_overrides_regardless_of_age() {
        let tmp_src = TempDir::new().unwrap();
        let src_kb = make_kb(
            &tmp_src,
            &[(
                "SEARCH-abc.md",
                "2026-01-01 09:00:00", // 很旧
                "rh_ft_search",
                "incoming",
                "incoming body",
            )],
        ...
        let ir = import_from_zip(
            ...
            ImportOptions {
                zip_path: out_zip,
                on_conflict: ConflictStrategy::AlwaysOverwrite,
            },
        )
        .unwrap();
        assert_eq!(ir.overwritten, 1);
    ...
    #[test]
    fn export_with_yuandian_cache_only_false_returns_unsupported() {
        let tmp = TempDir::new().unwrap();
        let kb = make_kb(&tmp, &[]);
        let err = export_to_zip(
            &kb,
            ExportOptions {
                yuandian_cache_only: false,
                output_path: tmp.path().join("x.zip"),
                include_readme: false,
                exporter_version: "test".to_string(),
            },
        )
        .unwrap_err();
        assert!(format!("{:?}", err).contains("yuandian_cache_only"));
    }

    #[test]
    fn import_zip_without_manifest_returns_error() {
        let tmp = TempDir::new().unwrap();
        let kb = make_kb(&tmp, &[]);
    ...
    #[test]
    fn export_empty_kb_produces_valid_empty_zip() {
        let tmp = TempDir::new().unwrap();
        let kb = make_kb(&tmp, &[]);
        ...
        .unwrap();
        assert_eq!(er.total_items, 0);
        // 反向导入(应该 total_in_zip=0)
        let tmp2 = TempDir::new().unwrap();
        let kb2 = make_kb(&tmp2, &[]);
        let ir = import_from_zip(
            &kb2,
            ImportOptions {
                zip_path: out,
        ...
        .unwrap();
        assert_eq!(ir.total_in_zip, 0);
        assert_eq!(ir.added, 0);
    }

    #[test]
    fn manifest_round_trip_serialization() {
        // 确保 Manifest serde 没漏字段 — 不会在新加字段时反序列化崩
        let m = Manifest {
            exported_at: "2026-05-28T10:00:00+08:00".to_string(),
            exporter_version: "0.1.16".to_string(),
            yuandian_cache_only: true,
            total_items: 1,
            total_size_bytes: 123,
            items: vec![ManifestItem {
                path: "yuandian-cache/SEARCH-abc.md".to_string(),
                cached_at: "2026-05-28 10:00:00".to_string(),
                size_bytes: 123,
                query_type: "rh_ft_search".to_string(),
        ...
        assert_eq!(back.items[0].query_type, "rh_ft_search");
    }
}
