...
async fn accept_memory_candidate(
    ...
    db::case_memory::accept_memory_candidate(&pool, &case_id, &candidate_id, input, "local-user")
        .await
}
...
async fn preview_legal_skill_diff(
    ...
    chat::legal_skills::preview_package_diff(pool.inner(), &current_skill_id, &target_skill_id)
        .await
        .map_err(|error| error.to_string())
}
...
#[tauri::command]
async fn update_criminal_case_stage(
    pool: tauri::State<'_, SqlitePool>,
    case_id: String,
    stage: String,
) -> Result<db::criminal_cases::CriminalCaseProfile, String> {
    db::criminal_cases::update_criminal_case_stage(pool.inner(), &case_id, &stage).await
}
...
static FEISHU_CONNECT_LOCK: std::sync::OnceLock<tokio::sync::Mutex<()>> =
...
async fn connect_feishu_readonly(
    ...
    let _guard = FEISHU_CONNECT_LOCK
...
#[tauri::command]
async fn reauthorize_feishu_sync() -> Result<feishu_oauth::FeishuOAuthStatus, String> {
    let _guard = FEISHU_CONNECT_LOCK
        .get_or_init(|| tokio::sync::Mutex::new(()))
        .try_lock()
        .map_err(|_| "FEISHU_OAUTH_IN_PROGRESS: 已有一个飞书授权窗口正在等待完成".to_string())?;
    ...
    if app_id.is_empty() {
        return Err("FEISHU_AUTH_REQUIRED: 请先配置飞书 App ID".to_string());
    }
    feishu_oauth::reauthorize_with_stored_secret(app_id, |url| {
        tauri_plugin_opener::open_url(url, None::<&str>).map_err(|_| ())
    })
    .await
    .map_err(feishu_oauth_error)
...
async fn pull_feishu_sync_preview_locked(
    ...
    let app_id = current
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| "FEISHU_AUTH_REQUIRED: 请先连接飞书案件同步授权".to_string())?;
...
async fn update_workflow_status(
    ...
    cases_db::update_workflow_status(pool.inner(), &case_id, status.as_deref()).await
}

#[derive(Deserialize)]
struct FeishuResolveFieldInput {
    preview_id: String,
    resolution: String,
}
...
/// 逐项处理进展、阶段或联系人差异；拉取本身不会触碰业务表。
#[tauri::command]
async fn resolve_feishu_sync_entity(
    pool: tauri::State<'_, SqlitePool>,
    input: FeishuResolveFieldInput,
) -> Result<(), String> {
    device_sync::feishu_binding_lifecycle::run_explicit_action(|| {
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            db::criminal_workspace::get_criminal_defense_workspace,
            db::criminal_workspace::list_criminal_review_notes,
            db::criminal_workspace::upsert_criminal_review_note,
            db::criminal_workspace::review_criminal_review_note,
            db::criminal_workspace::delete_criminal_review_note,
            db::criminal_workspace::list_criminal_evidence_items,
            db::criminal_workspace::upsert_criminal_evidence_item,
            db::criminal_workspace::review_criminal_evidence_item,
            db::criminal_workspace::delete_criminal_evidence_item,
            db::criminal_workspace::list_criminal_issues,
            db::criminal_workspace::upsert_criminal_issue,
            db::criminal_workspace::review_criminal_issue,
            db::criminal_workspace::delete_criminal_issue,
            db::criminal_workspace::upsert_criminal_issue_evidence_link,
            db::criminal_workspace::list_criminal_issue_evidence_links,
            db::criminal_workspace::review_criminal_issue_evidence_link,
            db::criminal_workspace::delete_criminal_issue_evidence_link,
            db::criminal_workspace::upsert_criminal_source_citation,
            db::criminal_workspace::list_criminal_source_citations,
            db::criminal_workspace::delete_criminal_source_citation,
            db::criminal_workspace::refresh_criminal_source_integrity,
            db::criminal_workspace::get_criminal_analysis_capabilities,
            db::criminal_workspace::start_criminal_analysis,
            db::criminal_workspace::get_criminal_analysis_run,
            db::criminal_workspace::list_criminal_analysis_runs,
            db::criminal_workspace::list_criminal_analysis_findings,
            db::criminal_workspace::upsert_criminal_analysis_finding,
            db::criminal_workspace::review_criminal_analysis_finding,
            db::criminal_workspace::create_criminal_draft,
            db::criminal_workspace::create_criminal_draft_version,
            db::criminal_workspace::list_criminal_drafts,
            db::criminal_workspace::get_criminal_draft,
            db::criminal_workspace::archive_criminal_draft,
            db::criminal_workspace::submit_criminal_draft_version_for_review,
            db::criminal_workspace::approve_criminal_draft_version,
            db::criminal_workspace::return_criminal_draft_version,
            db::criminal_workspace::export_criminal_draft_version,
            db::criminal_workspace::link_criminal_workspace_artifact_to_task,
            db::criminal_workspace::unlink_criminal_workspace_artifact_from_task,
            ...
            resolve_feishu_sync_field,
            resolve_feishu_sync_entity,
            get_feishu_connection_status,
            connect_feishu_readonly,
            reauthorize_feishu_sync,
