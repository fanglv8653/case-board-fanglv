...
mod tests {
    ...
    #[test]
    fn roster_sign_verify_roundtrip_and_tamper() {
        let r = roster_two();
        let signed = SignedRoster::sign(&r, "secret-a").unwrap();
        assert_eq!(signed.verify("secret-a").unwrap(), r);
        // 错密钥(隔壁团队)→ 拒
        assert!(signed.verify("secret-b").is_err());
        // 篡改内容 → 拒
        let mut tampered = signed.clone();
        tampered.roster_json = tampered.roster_json.replace("李四", "赵六");
        assert!(tampered.verify("secret-a").is_err());
    }
    ...
    fn view_permissions() {
        let r = roster_two();
        // 团队长恒看全队
        assert!(r.can_view("leader", "m1") && r.can_view("leader", "m2"));
        // m1 配了自定义范围:能看 m2,看不到 leader
        assert!(r.can_view("m1", "m2"));
        assert!(!r.can_view("m1", "leader"));
        // 自己恒可见
        assert!(r.can_view("m1", "m1"));
        // view=None 默认全队可见
        assert!(r.can_view("m2", "leader") && r.can_view("m2", "m1"));
        // 不在名单的人(被踢/外人)什么都看不到
        assert!(!r.can_view("ghost", "m1"));
    }

    #[test]
    fn edit_permissions_require_view() {
        let r = roster_two();
        assert!(r.can_edit("leader", "m2"), "团队长恒可编辑");
        assert!(r.can_edit("m1", "m2"), "明确授了 edit");
        assert!(!r.can_edit("m2", "m1"), "没授 edit 不能编辑");
        assert!(!r.can_edit("m1", "leader"), "看都看不到,更不能编辑");
    }

    #[test]
    fn merge_rule_seq_wins() {
        assert!(should_replace(None, 1), "没见过 → 收");
        assert!(should_replace(Some(3), 5), "更新 → 收");
        assert!(!should_replace(Some(5), 5), "同版本 → 不动");
        assert!(
            !should_replace(Some(5), 3),
            "旧的 → 拒(时钟漂移也不怕,seq 说了算)"
        );
    }
