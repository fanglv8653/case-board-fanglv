...
mod tests {
    ...
    async fn mem_pool() -> SqlitePool {
        init_pool(":memory:").await.expect("内存库")
    }
    ...
    #[tokio::test]
    async fn state_roundtrip_and_kicked_notice_once() {
        let pool = mem_pool().await;
        assert!(get_state(&pool, "x").await.unwrap().is_none());
        set_state(&pool, "x", "1").await.unwrap();
        set_state(&pool, "x", "2").await.unwrap();
        assert_eq!(get_state(&pool, "x").await.unwrap().unwrap(), "2");

        set_kicked_notice(&pool, "测试团队").await.unwrap();
        assert_eq!(
            take_kicked_notice(&pool).await.unwrap().unwrap(),
            "测试团队"
        );
        assert!(take_kicked_notice(&pool).await.unwrap().is_none(), "一次性");
    }

    #[tokio::test]
    async fn merge_respects_seq_rule() {
        let pool = mem_pool().await;
        let env = |seq: i64, name: &str| SnapshotEnvelope {
            member_id: "m1".into(),
            name: name.into(),
            seq,
        ...
        assert_eq!(merge_snapshots(&pool, &[env(3, "李四")]).await.unwrap(), 1);
        // 旧的拒收
        assert_eq!(merge_snapshots(&pool, &[env(2, "旧")]).await.unwrap(), 0);
        // 新的覆盖
        assert_eq!(merge_snapshots(&pool, &[env(5, "新")]).await.unwrap(), 1);
        let all = load_all_snapshots(&pool).await.unwrap();
        assert_eq!(all.len(), 1);
    ...
    #[tokio::test]
    async fn own_snapshot_seq_increments_and_persists() {
        let pool = mem_pool().await;
        let id = identity();
        let a = rebuild_own_snapshot(&pool, &id).await.unwrap();
        let b = rebuild_own_snapshot(&pool, &id).await.unwrap();
        assert_eq!(a.seq, 1);
        assert_eq!(b.seq, 2);
        let all = load_all_snapshots(&pool).await.unwrap();
        assert_eq!(all.len(), 1, "自己只占一行");
        // payload 可解析且为登记表形状
        let p: SnapshotPayload = serde_json::from_str(&all[0].payload).unwrap();
        assert!(p.cases.is_empty(), "空库无案件");
    }

    // ---------------- Phase 2:编辑请求 ----------------

    fn roster_with_edit_perm() -> Roster {
        Roster {
            team_id: "t1".into(),
            team_name: "测试团队".into(),
            seq: 1,
            members: vec![
                super::super::RosterMember {
                    member_id: "me".into(),
                    name: "张三".into(),
                    role: "leader".into(),
                    view: None,
    ...
    fn edit(id: &str, editor: &str, field: &str, value: &str, status: &str) -> TeamEdit {
        TeamEdit {
            id: id.into(),
            team_id: "t1".into(),
            editor_id: editor.into(),
            editor_name: "李四".into(),
            target_member_id: "me".into(),
            case_id: "case-1".into(),
            case_name: "测试案件".into(),
            field: field.into(),
            value: value.into(),
            prev_value: None,
            status: status.into(),
            created_at: "2026-06-10T01:00:00+08:00".into(),
            applied_at: None,
        }
    }

    async fn insert_test_case(pool: &SqlitePool) {
        sqlx::query(
            "INSERT INTO cases (id, name, source_folder, workflow_status)
             VALUES ('case-1', '测试案件', '/tmp/测试案件', 'trial')",
        )
        .execute(pool)
        .await
        .unwrap();
    ...
    #[tokio::test]
    async fn edit_merge_status_only_upgrades() {
        let pool = mem_pool().await;
        assert_eq!(
            merge_edits(&pool, &[edit("e1", "editor", "note", "v", "pending")])
                .await
                .unwrap(),
            1
        );
        ...
        // 升级 applied → 收
        assert_eq!(
            merge_edits(&pool, &[edit("e1", "editor", "note", "v", "applied")])
                .await
                .unwrap(),
            1
        );
        // 降级回 pending → 拒(状态只升不降,防老消息接力回流盖掉结果)
        assert_eq!(
            merge_edits(&pool, &[edit("e1", "editor", "note", "v", "pending")])
                .await
                .unwrap(),
        ...
        assert_eq!(
            merge_edits(
                &pool,
                &[edit("e2", "editor", "ai_summary_md", "x", "pending")]
            )
            .await
            .unwrap(),
            0
    ...
    #[tokio::test]
    async fn apply_edit_changes_case_and_keeps_prev_then_revert() {
        let pool = mem_pool().await;
        insert_test_case(&pool).await;
        let id = identity(); // member_id = "me"
        let roster = roster_with_edit_perm();
        merge_edits(
            &pool,
            &[edit(
                "e1",
        ...
        .await
        .unwrap();
        let applied = apply_my_pending_edits(&pool, &id, &roster).await.unwrap();
        assert_eq!(applied, 1);
        // 案件状态真的被改了 + prev 留痕
        let cur = sqlx::query_scalar::<_, Option<String>>(
            "SELECT workflow_status FROM cases WHERE id = 'case-1'",
        )
        .fetch_one(&pool)
        .await
        .unwrap();
        assert_eq!(cur.as_deref(), Some("executing"));
        let all = load_recent_edits(&pool).await.unwrap();
        assert_eq!(all[0].status, "applied");
        assert_eq!(all[0].prev_value.as_deref(), Some("trial"));
        // 撤销 → 恢复原值
        revert_edit(&pool, &id, "e1").await.unwrap();
        let cur = sqlx::query_scalar::<_, Option<String>>(
            "SELECT workflow_status FROM cases WHERE id = 'case-1'",
        )
        .fetch_one(&pool)
        .await
        .unwrap();
        assert_eq!(cur.as_deref(), Some("trial"));
        assert_eq!(
            load_recent_edits(&pool).await.unwrap()[0].status,
            "reverted"
        );
    }

    #[tokio::test]
    async fn apply_rejects_no_permission_and_missing_case() {
        let pool = mem_pool().await;
        insert_test_case(&pool).await;
        let id = identity();
        let roster = roster_with_edit_perm();
        // 无编辑权的人 → rejected,案件不动
        merge_edits(
            &pool,
            &[edit(
                "e1",
        ...
        .unwrap();
        // 案件不存在 → rejected
        let mut gone = edit("e2", "editor", "workflow_status", "executing", "pending");
        gone.case_id = "no-such-case".into();
        merge_edits(&pool, std::slice::from_ref(&gone))
            .await
            .unwrap();
        let applied = apply_my_pending_edits(&pool, &id, &roster).await.unwrap();
        assert_eq!(applied, 0);
        let statuses: Vec<String> = load_recent_edits(&pool)
            .await
            .unwrap()
            .into_iter()
            .map(|e| e.status)
        ...
        .await
        .unwrap();
        assert_eq!(cur.as_deref(), Some("trial"), "无权限编辑不能生效");
    }

    #[tokio::test]
    async fn clear_team_data_wipes_edits_too() {
        // 0.3.11 遗漏的回归:退团必须连 team_edits 一起清,防旧团队记录串进新团队
        let pool = mem_pool().await;
        merge_edits(&pool, &[edit("e1", "editor", "note", "v", "pending")])
            .await
            .unwrap();
        set_state(&pool, "x", "1").await.unwrap();
        clear_team_data(&pool).await.unwrap();
        assert!(load_recent_edits(&pool).await.unwrap().is_empty());
        assert!(load_all_snapshots(&pool).await.unwrap().is_empty());
        assert!(get_state(&pool, "x").await.unwrap().is_none());
    }

    #[tokio::test]
    async fn note_allowed_for_any_visible_member() {
        // 备注可见即可写(老板拍板"谁都可以写备注"):noperm 没有编辑权,但能留备注
        let pool = mem_pool().await;
        insert_test_case(&pool).await;
        let id = identity();
        let roster = roster_with_edit_perm();
        merge_edits(
            &pool,
            &[edit("e1", "noperm", "note", "记得带证据原件", "pending")],
        )
        .await
        .unwrap();
        assert_eq!(
            apply_my_pending_edits(&pool, &id, &roster).await.unwrap(),
            1
        );
        assert_eq!(load_recent_edits(&pool).await.unwrap()[0].status, "applied");
    }

    #[tokio::test]
    async fn latest_event_picks_most_recent_past() {
        // 时间轴混过去/未来:最新进展 = 已发生的最后一件;未来不算进展
        let dates = [
            SnapshotDate {
        ...
        let latest = dates
            ...
            .rfind(|d| d.date.len() >= 10 && d.date[..10] <= today[..])
            .cloned();
        assert_eq!(latest.unwrap().event, "一审开庭");
    }

    #[tokio::test]
    async fn note_applies_without_touching_cases() {
        let pool = mem_pool().await;
        insert_test_case(&pool).await;
        let id = identity();
        let roster = roster_with_edit_perm();
        merge_edits(
            &pool,
            &[edit("e1", "editor", "note", "下周三前提交证据", "pending")],
        )
        .await
        .unwrap();
        assert_eq!(
    ...
    #[test]
    fn parties_and_dates_parse_defensively() {
        assert!(parse_name_list(None).is_empty());
        assert!(parse_name_list(Some("not json")).is_empty());
        let names = parse_name_list(Some(r#"["甲",{"name":"乙"},""]"#));
        assert_eq!(names, vec!["甲".to_string(), "乙".to_string()]);

        let dates = parse_key_dates(Some(
            r#"[{"date":"2026-08-01","event":"续封到期"},{"event":"没日期"}]"#,
        ));
        assert_eq!(dates.len(), 1);
        assert_eq!(dates[0].event, "续封到期");
    }
}
