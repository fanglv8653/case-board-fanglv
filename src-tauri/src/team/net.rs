...
mod tests {
    ...
    /// 极简 HTTP 解析:happy path + 分片到达 + 超限拒绝,走真 TcpStream(127.0.0.1)。
    #[tokio::test]
    async fn http_parse_roundtrip() {
        let listener = TcpListener::bind(("127.0.0.1", 0)).await.unwrap();
        let addr = listener.local_addr().unwrap();
        let server = tokio::spawn(async move {
            let (mut s, _) = listener.accept().await.unwrap();
            let req = read_http_request(&mut s).await.unwrap();
            assert_eq!(req.path, "/exchange");
            assert_eq!(req.headers.get("x-team-auth").unwrap(), "abc");
            assert_eq!(req.body, b"{\"k\":1}");
            write_http(&mut s, 200, "{\"ok\":true}", Some("mac"))
                .await
                .unwrap();
        });
        ...
        // 分两段发,验证粘包/分片处理
        c.write_all(
            b"POST /exchange HTTP/1.1\r\nContent-Length: 7\r\nx-team-auth: abc\r\n\r\n{\"k\"",
        )
        .await
        .unwrap();
        tokio::time::sleep(Duration::from_millis(30)).await;
        c.write_all(b":1}").await.unwrap();
        let mut out = Vec::new();
        c.read_to_end(&mut out).await.unwrap();
        let text = String::from_utf8_lossy(&out);
        assert!(text.starts_with("HTTP/1.1 200"));
        assert!(text.contains("x-team-auth: mac"));
        assert!(text.ends_with("{\"ok\":true}"));
        server.await.unwrap();
    }

    #[tokio::test]
    async fn http_parse_rejects_get() {
        let listener = TcpListener::bind(("127.0.0.1", 0)).await.unwrap();
        let addr = listener.local_addr().unwrap();
        let server = tokio::spawn(async move {
            let (mut s, _) = listener.accept().await.unwrap();
            assert!(read_http_request(&mut s).await.is_err());
        });
        let mut c = TcpStream::connect(addr).await.unwrap();
