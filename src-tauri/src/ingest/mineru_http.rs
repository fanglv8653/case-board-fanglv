...
mod tests {
    ...
    #[test]
    fn api_envelope_deserializes_ok_responses() {
        // 上传成功的响应
        let json = r#"{"code":0,"data":{"batch_id":"abc","file_urls":["https://x"]},"msg":"ok"}"#;
        let env: ApiEnvelope<BatchUploadData> = serde_json::from_str(json).unwrap();
        assert_eq!(env.code, 0);
        let data = env.data.unwrap();
        assert_eq!(data.batch_id, "abc");
        assert_eq!(data.file_urls.len(), 1);
    }

    #[test]
    fn api_envelope_deserializes_done_state() {
        let json = r#"{"code":0,"data":{"batch_id":"abc","extract_result":[{"file_name":"x.pdf","state":"done","full_zip_url":"https://x.zip","err_msg":""}]},"msg":"ok"}"#;
        let env: ApiEnvelope<BatchResultData> = serde_json::from_str(json).unwrap();
        let data = env.data.unwrap();
        let result = &data.extract_result[0];
        assert_eq!(result.state, "done");
        assert_eq!(result.full_zip_url.as_deref(), Some("https://x.zip"));
    }

    #[test]
    fn api_envelope_handles_running_state() {
        // 轮询过程中可能没 full_zip_url(还在 running)
        let json = r#"{"code":0,"data":{"batch_id":"abc","extract_result":[{"file_name":"x.pdf","state":"running"}]}}"#;
        let env: ApiEnvelope<BatchResultData> = serde_json::from_str(json).unwrap();
        let data = env.data.unwrap();
        let result = &data.extract_result[0];
