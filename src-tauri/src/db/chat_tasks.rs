...
mod tests {
    ...
    async fn make_case(pool: &SqlitePool) -> String {
        ...
        sqlx::query("INSERT INTO cases (id, name, case_type, source_folder) VALUES (?, ?, ?, ?)")
            .bind(&case_id)
            .bind("chat_tasks 测试案件")
            .bind("诉讼")
            .bind(format!("/tmp/chat_tasks_test/{}", case_id))
            .execute(pool)
            .await
            .unwrap();
        case_id
    ...
    fn new_task<'a>(id: &'a str, case_id: &'a str, message_id: &'a str) -> NewChatTask<'a> {
        NewChatTask {
            id,
            case_id,
            message_id,
            task_type: "compile_legal_basis",
            status: "planning",
    ...
    async fn create_then_get_round_trip() {
        ...
        create_chat_task(&pool, new_task(&task_id, &case_id, &msg_id))
            .await
            .unwrap();

        let got = get_chat_task(&pool, &task_id).await.unwrap().unwrap();
        assert_eq!(got.case_id, case_id);
        assert_eq!(got.task_type, "compile_legal_basis");
        assert_eq!(got.status, "planning");
        assert_eq!(got.yuandian_credits_used, 0);
        assert_eq!(got.kb_hits, 0);
        assert!(got.finished_at.is_none());
        // started_at 应该是 RFC3339 串,含 'T'
        assert!(got.started_at.contains('T'));
    }
    ...
    async fn create_rejects_invalid_status() {
        let pool = init_pool(":memory:").await.unwrap();
        let case_id = make_case(&pool).await;
        let mut t = new_task("t1", &case_id, "m1");
        t.status = "bogus";
        let err = create_chat_task(&pool, t).await.unwrap_err();
        assert!(format!("{}", err).contains("invalid chat_task status"));
    }

    #[tokio::test]
    async fn get_returns_none_when_missing() {
        let pool = init_pool(":memory:").await.unwrap();
        let got = get_chat_task(&pool, "no-such-id").await.unwrap();
        assert!(got.is_none());
    }

    #[tokio::test]
    async fn update_only_writes_some_fields() {
        let pool = init_pool(":memory:").await.unwrap();
        let case_id = make_case(&pool).await;
        let task_id = uuid::Uuid::new_v4().to_string();
        create_chat_task(&pool, new_task(&task_id, &case_id, "m"))
            .await
            .unwrap();

        // 第一轮:只改 plan_json + status
        update_chat_task(
            &pool,
            &task_id,
            UpdateChatTask {
                status: Some("executing"),
                plan_json: Some(r#"[{"sub":"查违约金条款"}]"#),
                ..Default::default()
            },
        )
        .await
        .unwrap();
        let t1 = get_chat_task(&pool, &task_id).await.unwrap().unwrap();
        assert_eq!(t1.status, "executing");
        assert_eq!(t1.plan_json.as_deref(), Some(r#"[{"sub":"查违约金条款"}]"#));
        assert!(t1.tool_calls_json.is_none());

        // 第二轮:只改 tool_calls_json + tokens,前面已经写过的 plan_json 应该保留
        update_chat_task(
            &pool,
            &task_id,
            UpdateChatTask {
                tool_calls_json: Some(r#"[{"tool":"search_laws"}]"#),
                prompt_tokens: Some(1234),
                completion_tokens: Some(5678),
                ..Default::default()
            },
        )
        .await
        .unwrap();
        let t2 = get_chat_task(&pool, &task_id).await.unwrap().unwrap();
        assert_eq!(t2.status, "executing", "status 没改应保留");
        assert_eq!(
            t2.plan_json.as_deref(),
            Some(r#"[{"sub":"查违约金条款"}]"#),
            "plan_json 没改应保留"
        );
        assert_eq!(
            t2.tool_calls_json.as_deref(),
            Some(r#"[{"tool":"search_laws"}]"#)
        );
        assert_eq!(t2.prompt_tokens, Some(1234));
        assert_eq!(t2.completion_tokens, Some(5678));
    }
    ...
    #[tokio::test]
    async fn update_with_empty_patch_is_noop() {
        let pool = init_pool(":memory:").await.unwrap();
        let case_id = make_case(&pool).await;
        let task_id = uuid::Uuid::new_v4().to_string();
        create_chat_task(&pool, new_task(&task_id, &case_id, "m"))
            .await
            .unwrap();
        // 空 patch 不应该报错也不应该改任何字段
        update_chat_task(&pool, &task_id, UpdateChatTask::default())
            .await
            .unwrap();
        let t = get_chat_task(&pool, &task_id).await.unwrap().unwrap();
        assert_eq!(t.status, "planning");
    }

    #[tokio::test]
    async fn finish_writes_terminal_status_and_finished_at() {
        let pool = init_pool(":memory:").await.unwrap();
        let case_id = make_case(&pool).await;
        let task_id = uuid::Uuid::new_v4().to_string();
        create_chat_task(&pool, new_task(&task_id, &case_id, "m"))
            .await
            .unwrap();
        finish_chat_task(&pool, &task_id, "done", None)
            .await
            .unwrap();
        let t = get_chat_task(&pool, &task_id).await.unwrap().unwrap();
        assert_eq!(t.status, "done");
        assert!(t.finished_at.as_deref().unwrap().contains('T'));
        assert!(t.error_short.is_none());
    }
    ...
    async fn finish_rejects_non_terminal_status() {
        ...
        create_chat_task(&pool, new_task(&task_id, &case_id, "m"))
            .await
            .unwrap();
        let err = finish_chat_task(&pool, &task_id, "executing", None)
            .await
            .unwrap_err();
    ...
    #[tokio::test]
    async fn mark_failed_writes_error_short() {
        let pool = init_pool(":memory:").await.unwrap();
        let case_id = make_case(&pool).await;
        let task_id = uuid::Uuid::new_v4().to_string();
        create_chat_task(&pool, new_task(&task_id, &case_id, "m"))
            .await
            .unwrap();
        mark_failed(&pool, &task_id, "上次任务异常中断,可点击重试")
            .await
            .unwrap();
        let t = get_chat_task(&pool, &task_id).await.unwrap().unwrap();
        assert_eq!(t.status, "failed");
        assert_eq!(
            t.error_short.as_deref(),
            Some("上次任务异常中断,可点击重试")
        );
    ...
    #[tokio::test]
    async fn list_by_case_orders_desc_by_started_at() {
        let pool = init_pool(":memory:").await.unwrap();
        let case_id = make_case(&pool).await;
        for i in 0..3 {
            let id = format!("t{}", i);
            create_chat_task(&pool, new_task(&id, &case_id, "m"))
                .await
                .unwrap();
            // 让 started_at 时间错开(SQLite 同微秒可能并列,这里手动 sleep 1ms)
            tokio::time::sleep(std::time::Duration::from_millis(2)).await;
        }
        let v = list_chat_tasks_by_case(&pool, &case_id, None)
            .await
            .unwrap();
        assert_eq!(v.len(), 3);
        assert_eq!(v[0].id, "t2");
        ...
        // limit 生效
        let v2 = list_chat_tasks_by_case(&pool, &case_id, Some(2))
            .await
            .unwrap();
        assert_eq!(v2.len(), 2);
        assert_eq!(v2[0].id, "t2");
    }

    #[tokio::test]
    async fn find_orphans_excludes_terminal_and_recent_active() {
        let pool = init_pool(":memory:").await.unwrap();
        let case_id = make_case(&pool).await;

        // 直接 INSERT 控制 started_at(测试要造旧任务)
        let old_id = "old_active";
        let old_started = (Utc::now() - Duration::seconds(600)).to_rfc3339();
        sqlx::query(
            "INSERT INTO chat_tasks (id, case_id, message_id, task_type, status, started_at) \
             VALUES (?, ?, ?, ?, ?, ?)",
        )
        .bind(old_id)
        .bind(&case_id)
        .bind("m_old")
        .bind("compile_legal_basis")
        .bind("executing")
        .bind(&old_started)
        .execute(&pool)
        .await
        ...
        // 一个最近的 active(不应是 orphan)
        create_chat_task(&pool, new_task("fresh_active", &case_id, "m_fresh"))
            .await
            .unwrap();

        // 一个已完结(永远不是 orphan,哪怕很旧)
        let done_started = (Utc::now() - Duration::seconds(9999)).to_rfc3339();
        sqlx::query(
            "INSERT INTO chat_tasks (id, case_id, message_id, task_type, status, started_at, finished_at) \
             VALUES (?, ?, ?, ?, 'done', ?, ?)",
        )
        .bind("old_done")
        .bind(&case_id)
        ...
        // 阈值 5 分钟 → 只能捞到 old_active
        let orphans = find_orphans(&pool, Duration::seconds(300)).await.unwrap();
        assert_eq!(orphans.len(), 1);
        assert_eq!(orphans[0].id, "old_active");
    }

    #[tokio::test]
    async fn resume_orphaned_chat_tasks_marks_all_as_failed() {
        let pool = init_pool(":memory:").await.unwrap();
        let case_id = make_case(&pool).await;

        // 造 2 个旧 active
        for (id, status) in [("o1", "planning"), ("o2", "verifying")] {
            let started = (Utc::now() - Duration::seconds(600)).to_rfc3339();
            sqlx::query(
                "INSERT INTO chat_tasks (id, case_id, message_id, task_type, status, started_at) \
                 VALUES (?, ?, ?, ?, ?, ?)",
            )
        ...
        // 1 个最近 active(不应被标 failed)
        create_chat_task(&pool, new_task("fresh", &case_id, "m"))
            .await
            .unwrap();

        let n = resume_orphaned_chat_tasks(&pool, 300).await;
        assert_eq!(n, 2);

        for id in ["o1", "o2"] {
            let t = get_chat_task(&pool, id).await.unwrap().unwrap();
            assert_eq!(t.status, "failed");
            assert_eq!(
                t.error_short.as_deref(),
                Some("上次任务异常中断,可点击重试")
        ...
        let fresh = get_chat_task(&pool, "fresh").await.unwrap().unwrap();
        assert_eq!(fresh.status, "planning");
    }

    #[test]
    fn status_helpers() {
        assert!(is_active_status("planning"));
        assert!(is_active_status("executing"));
        assert!(is_active_status("synthesizing"));
        assert!(is_active_status("verifying"));
        ...
        assert!(!is_terminal_status("planning"));
    ...
    #[tokio::test]
    async fn case_delete_cascades_chat_tasks() {
        let pool = init_pool(":memory:").await.unwrap();
        let case_id = make_case(&pool).await;
        create_chat_task(&pool, new_task("t-cascade", &case_id, "m"))
            .await
            .unwrap();
        sqlx::query("DELETE FROM cases WHERE id = ?")
            ...
            .await
            .unwrap();
        let got = get_chat_task(&pool, "t-cascade").await.unwrap();
        assert!(got.is_none(), "案件删除应级联清掉 chat_tasks");
    }
}
