...
async fn fresh_database_reaches_current_lineage_and_all_frozen_sentinels() {
    ...
    assert_eq!(actual_versions.len(), 67);
    assert_eq!(actual_versions.last(), Some(&68));
    ...
    assert_eq!(migrated_count, embedded_versions.len() as i64);
...
async fn authorized_preflight_then_missing_v36_fails_without_forging_history() {
    ...
    assert_eq!(migration_state, (0, 68));
    pool.close().await;
}
