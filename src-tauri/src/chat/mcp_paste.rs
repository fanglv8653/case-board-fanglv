...
mod tests {
    ...
    /// 元典 WorkBuddy 文档样式:外层注释行 + mcpServers 包裹 + 占位符 key,混在说明文字里。
    #[test]
    fn parses_yuandian_workbuddy_style() {
        let text = r#"粘贴右侧 MCP CONFIG JSON(把 YOUR_API_KEY 换成真实 Key)· 点击「保存」
// Streamable HTTP 方式 · 三个 Server 一次装上
{
  "mcpServers": {
    "yuandian-law": {
      "type": "http",
...
确认元典 MCP 服务已启用"#;
        let r = parse_pasted_config(text).unwrap();
        assert_eq!(r.servers.len(), 2);
        // JSON object 遍历顺序取决于 serde_json 特性开关 → 只验集合不验顺序
        let names: Vec<_> = r.servers.iter().map(|s| s.name.as_str()).collect();
        assert!(names.contains(&"yuandian-law") && names.contains(&"yuandian-case"));
        let law = r.servers.iter().find(|s| s.name == "yuandian-law").unwrap();
        match &law.transport {
            McpTransport::Http { url, headers } => {
                assert_eq!(url, "https://open.chineselaw.com/mcp/law/stream");
                assert_eq!(headers.get("Authorization").unwrap(), "Bearer YOUR_API_KEY");
            }
            _ => panic!("应是 http"),
        }
        // 占位符警告:两个 server 都该被点名
        assert_eq!(r.warnings.len(), 2);
        assert!(r.warnings[0].contains("YOUR_API_KEY"));
    }

    /// 企查查聊天框样式:裸 name→def map,无 mcpServers 包裹、def 无 type 字段。
    #[test]
    fn parses_qcc_bare_map_style() {
        let text = r#"请配置企查查 MCP
{
  "qcc-company": {
    "url": "https://agent.qcc.com/mcp/company/stream",
    "headers": { "Authorization": "Bearer M8abcdef" }
  },
  "qcc-risk": {
    ...
    "headers": { "Authorization": "Bearer M8abcdef" }
        ...
        let r = parse_pasted_config(text).unwrap();
        assert_eq!(r.servers.len(), 2);
        assert!(r.servers.iter().all(|s| matches!(&s.transport, McpTransport::Http { headers, .. } if headers.contains_key("Authorization"))));
        // 真实样式的 key 不是占位符 → 无警告
        assert!(r.warnings.is_empty());
    }

    /// 北大法宝 Claude Code 命令行样式:`\` 续行 + --scope + --header 冒号分隔。
    #[test]
    fn parses_pkulaw_claude_mcp_add_command() {
        let text = "claude mcp add --transport http pkulaw-law-search --scope user \\\n  https://apim-gateway.pkulaw.com/mcp-law-search-service \\\n  --header \"Authorization: Bearer YOUR_ACCESS_TOKEN\"";
        let r = parse_pasted_config(text).unwrap();
        assert_eq!(r.servers.len(), 1);
        assert_eq!(r.servers[0].name, "pkulaw-law-search");
        ...
        assert_eq!(r.warnings.len(), 1, "占位符 token 应有警告");
        assert!(r.warnings[0].contains("YOUR_ACCESS_TOKEN"));
    }

    /// Claude Desktop 经典 stdio 样式。
    #[test]
    fn parses_stdio_mcp_servers_json() {
        let text = r#"{
  "mcpServers": {
    "filesystem": {
      "command": "npx",
    ...
    /// 单个裸 def(无名字)→ 从 URL 推导名字。
    #[test]
    fn derives_name_for_bare_single_def() {
        let text = r#"{ "type": "http", "url": "https://open.chineselaw.com/mcp/company/stream",
                       "headers": { "Authorization": "Bearer k" } }"#;
        let r = parse_pasted_config(text).unwrap();
        assert_eq!(r.servers.len(), 1);
        assert_eq!(r.servers[0].name, "company"); // 滤掉 mcp/stream 后取最后一段
    }

    /// 同页同名(HTTP tab + npx tab 都被复制)→ 按 name 去重保留先出现的。
    #[test]
    fn dedups_by_name_keeps_first() {
        let text = r#"
claude mcp add --transport http dual https://x.example/mcp --header "Authorization: Bearer k"
{ "mcpServers": { "dual": { "command": "npx", "args": ["-y", "dual-server"] } } }
"#;
        let r = parse_pasted_config(text).unwrap();
        assert_eq!(r.servers.len(), 1);
        assert!(
            matches!(r.servers[0].transport, McpTransport::Http { .. }),
            "命令行先解析,应保留 http 版本"
        );
    }

    #[test]
    fn garbage_text_errors_readably() {
        let err = parse_pasted_config("随便一段话,没有任何配置。{\"foo\": 1}").unwrap_err();
        assert!(err.contains("没有识别出"), "错误要人读: {err}");
    }

    #[test]
    fn shell_tokens_handles_quotes() {
        let t = shell_tokens(r#"--header "Authorization: Bearer a b" name https://u"#);
        assert_eq!(t[1], "Authorization: Bearer a b");
        assert_eq!(t[2], "name");
    }

    #[test]
    fn placeholder_detection_variants() {
        assert_eq!(
            find_placeholder("Bearer YOUR_API_KEY").unwrap(),
            "YOUR_API_KEY"
        );
        assert!(find_placeholder("Bearer <在此填token>").is_some());
        assert!(find_placeholder("Bearer sk-real-1234").is_none());
    }
}
