...
mod tests {
    ...
    #[test]
    fn enterprise_search_requires_name() {
        let s = EnterpriseSearch.parameters_schema();
        assert!(s["required"]
            .as_array()
            .unwrap()
            .iter()
    ...
    #[test]
    fn entity_from_args_id_or_uscc() {
        let with_id = json!({"id": "x-001"});
        let eid = entity_from_args(&with_id).unwrap();
        assert!(matches!(eid, EntityId::Id(_)));

        let with_uscc = json!({"tyshxydm": "91320200MA1XXXXXX0"});
        let eid = entity_from_args(&with_uscc).unwrap();
        assert!(matches!(eid, EntityId::Uscc(_)));

        let empty = json!({});
        assert!(entity_from_args(&empty).is_err());
    }

    #[test]
    fn enterprise_annual_report_requires_year() {
        let s = EnterpriseAnnualReport.parameters_schema();
        assert!(s["required"]
            .as_array()
            .unwrap()
            .iter()
