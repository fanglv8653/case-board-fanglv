...
mod tests {
    ...
    #[test]
    fn search_local_kb_requires_keyword() {
        let s = SearchLocalKb.parameters_schema();
        assert!(s["required"]
            .as_array()
            .unwrap()
            .iter()
    ...
    #[test]
    fn read_kb_file_requires_relative_path() {
        let s = ReadKbFile.parameters_schema();
        assert!(s["required"]
            .as_array()
            .unwrap()
            .iter()
            .any(|v| v == "relative_path"));
    }

    #[test]
    fn parse_scopes_defaults_to_5() {
        // notes + companies + sources + topics + gap_log(P2:加 companies)
        let v = parse_scopes(None, false);
        assert_eq!(v.len(), 5);
    }

    #[test]
    fn parse_scopes_explicit_subset() {
        let raw = json!(["notes", "sources"]);
        let v = parse_scopes(Some(&raw), false);
        assert_eq!(v.len(), 2);
    }

    #[test]
    fn parse_scopes_include_yuandian_appends() {
        let v = parse_scopes(None, true);
        assert_eq!(v.len(), 6); // 默认 5 + yuandian_cache
    }
}
