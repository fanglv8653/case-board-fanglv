...
mod tests {
    ...
    fn name_and_schema() {
        let t = SaveArtifact;
        assert_eq!(t.name(), "save_artifact");
        let s = t.parameters_schema();
        let req = s["required"].as_array().unwrap();
        for k in ["doc_type", "title", "content_md"] {
            assert!(req.iter().any(|v| v == k), "缺必填 {}", k);
        }
        // V0.3 · 白名单已开放:doc_type 是自由 string,**不再有 enum 限制**(任意文书类型可写)
        assert_eq!(s["properties"]["doc_type"]["type"], "string");
        assert!(
            s["properties"]["doc_type"]["enum"].is_null(),
            "doc_type 不该再有 enum(白名单已开放)"
        );
    }

    #[tokio::test]
    async fn persist_accepts_arbitrary_doc_type() {
        // 白名单开放:写一个固定结构之外的类型(答辩状)应正常落库,category=该类型。
        use crate::db::cases::{create_case, NewCase};
        use crate::db::init_pool;
        let _env_guard = crate::chat::HOME_ENV_LOCK.lock().await;
        let tmp = tempfile::tempdir().unwrap();
        std::env::set_var("HOME", tmp.path());
        std::env::set_var("XDG_DATA_HOME", tmp.path().join("share2"));
        ...
        let case = create_case(
            ...
            NewCase {
                name: "x".into(),
                case_type: "诉讼".into(),
                source_folder: "/tmp/x".into(),
            },
        )
        .await
        .unwrap();
        let doc_id = persist_filing(&pool, &case.id, "答辩状", "民事答辩状", "答辩如下…")
            .await
            .expect("任意 doc_type 应能落库");
        let cat: Option<String> = sqlx::query_scalar("SELECT category FROM documents WHERE id = ?")
            .bind(&doc_id)
            .fetch_one(&pool)
            .await
            .unwrap();
        assert_eq!(cat.as_deref(), Some("答辩状"));
    }

    #[test]
    fn description_substantial() {
        // §19.7:description 须 ≥400 字
        assert!(SaveArtifact.description().chars().count() >= 400);
    }

    #[test]
    fn safe_stem_strips_path_chars() {
        assert_eq!(safe_stem("民事/起诉状:测试"), "民事_起诉状_测试");
        assert!(!safe_stem("a/b\\c").contains('/'));
        assert!(safe_stem(&"长".repeat(100)).chars().count() <= 40);
    }

    #[tokio::test]
    async fn persist_creates_document_row() {
        use crate::db::cases::{create_case, NewCase};
        use crate::db::init_pool;

        // 临时 HOME 防污染本机 app data(persist_filing 走 app_data_dir)
        let _env_guard = crate::chat::HOME_ENV_LOCK.lock().await;
        let tmp = tempfile::tempdir().unwrap();
        std::env::set_var("HOME", tmp.path());
        ...
        let case = create_case(
            &pool,
            NewCase {
                name: "filing test".into(),
                case_type: "诉讼".into(),
                source_folder: "/tmp/filing".into(),
            },
        )
        .await
        .unwrap();

        let doc_id = persist_filing(
            &pool,
            &case.id,
            "民事起诉状",
            "测试起诉状",
        ...
        let row: (Option<String>, String, bool, String) = sqlx::query_as(
            "SELECT category, source, is_ai_artifact, filename FROM documents WHERE id = ?",
        )
        .bind(&doc_id)
        .fetch_one(&pool)
        .await
        ...
        assert_eq!(
            row.0.as_deref(),
            Some("民事起诉状"),
            "category 应为 doc_type"
        );
        assert_eq!(row.1, "chat_artifact", "source 应为 chat_artifact");
        assert!(row.2, "is_ai_artifact 应为真(不回喂上下文)");
        assert!(row.3.contains("测试起诉状"), "filename 含标题");

        // 文件落盘且含元信息头(导出时据此取 title)
        let path: String = sqlx::query_scalar("SELECT source_path FROM documents WHERE id = ?")
            .bind(&doc_id)
            .fetch_one(&pool)
            .await
            .unwrap();
        let md = std::fs::read_to_string(&path).unwrap();
        assert!(md.contains("<!-- filing · doc_type=民事起诉状"));
        assert!(md.contains("title=测试起诉状"));
        assert!(md.contains("判令"));
    }

    // ───────────────────── edit_artifact matcher 纯函数测试 ─────────────────────

    #[test]
    fn edit_artifact_name_schema_mutating() {
        let t = EditArtifact;
        assert_eq!(t.name(), "edit_artifact");
        assert!(t.is_mutating(), "edit_artifact 必须标 mutating(串行独占)");
        let s = t.parameters_schema();
        let req = s["required"].as_array().unwrap();
        for k in ["doc_id", "find", "replace"] {
            assert!(req.iter().any(|v| v == k), "缺必填 {}", k);
        }
        assert!(EditArtifact.description().chars().count() >= 400);
    }

    #[test]
    fn split_header_and_no_header() {
        let raw = "<!-- filing · doc_type=起诉状 · title=X · ts=2026 -->\n\n正文第一段\n正文第二段";
        let (h, b) = split_filing_header(raw);
        assert!(h.starts_with("<!-- filing"));
        assert!(h.ends_with("-->\n\n"), "头要含 --> 后的空白分隔: {:?}", h);
        assert_eq!(b, "正文第一段\n正文第二段");
        // 重组还原
        assert_eq!(format!("{}{}", h, b), raw);

        let plain = "没有头的纯正文";
        let (h2, b2) = split_filing_header(plain);
        assert_eq!(h2, "");
        assert_eq!(b2, plain);
    }

    #[test]
    fn locate_exact_unique_and_replace_keeps_rest() {
        let body = "诉讼请求:判令被告支付货款五十万元,并承担本案诉讼费。";
        let (s, e) = locate_match(body, "五十万元", None, None).unwrap();
        assert_eq!(&body[s..e], "五十万元");
        let edited = format!("{}{}{}", &body[..s], "八十万元", &body[e..]);
        assert_eq!(
            edited,
            "诉讼请求:判令被告支付货款八十万元,并承担本案诉讼费。"
        );
    }

    #[test]
    fn locate_normalized_through_bold_marks() {
        // AI 给的 find 不带 **,文书里是加粗的 → 归一匹配命中,范围吸收两侧 **(不留悬空)
        let body = "本院认为,原告**主张赔偿**于法有据。";
        let (s, e) = locate_match(body, "原告主张赔偿", None, None).unwrap();
        assert_eq!(&body[s..e], "原告**主张赔偿**", "应吸收边界 ** 标记");
        let edited = format!("{}{}{}", &body[..s], "原告诉请赔偿", &body[e..]);
        assert_eq!(
            edited, "本院认为,原告诉请赔偿于法有据。",
            "替换后不留悬空 **"
        );
    }

    #[test]
    fn locate_normalized_collapses_whitespace() {
        // find 用单空格,body 里是换行 → 压空白后命中
        let body = "甲方:张三\n乙方:李四";
        let (s, e) = locate_match(body, "张三 乙方", None, None).unwrap();
        assert_eq!(&body[s..e], "张三\n乙方");
    }

    #[test]
    fn locate_ambiguous_errors() {
        let body = "金额为十万元,违约金为十万元。";
        assert_eq!(
            locate_match(body, "十万元", None, None),
            Err(LocateError::Ambiguous(2))
        );
    }

    #[test]
    fn locate_context_disambiguates() {
        let body = "金额为十万元,违约金为十万元。";
        // 用前文锚点定位第二个「十万元」
        let (s, e) = locate_match(body, "十万元", Some("违约金为"), None).unwrap();
        assert_eq!(&body[s..e], "十万元");
        assert!(s > 0 && body[..s].ends_with("违约金为"));
    }

    #[test]
    fn locate_not_found_errors() {
        let body = "诉讼请求:判令被告支付货款。";
        assert_eq!(
            locate_match(body, "不存在的片段", None, None),
            Err(LocateError::NotFound)
        );
    }

    #[test]
    fn surrounding_snippet_is_char_safe() {
        let s = "一二三四五六七八九十";
        let out = surrounding_snippet(s, 3 * 2, 3 * 4, 2); // 改「三四」
        assert!(out.contains("〖三四〗"));
        assert!(out.contains("一二") && out.contains("五六"));
    }

    // ───────────────────── apply_edit DB 测试 ─────────────────────

    async fn seed_artifact(pool: &sqlx::SqlitePool, case_id: &str) -> (String, String) {
        let doc_id = persist_filing(
            pool,
            case_id,
            "起诉状",
    ...
    #[tokio::test]
    async fn apply_edit_in_place_preserves_header() {
        use crate::db::cases::{create_case, NewCase};
        use crate::db::init_pool;
        let _env_guard = crate::chat::HOME_ENV_LOCK.lock().await;
        let tmp = tempfile::tempdir().unwrap();
        std::env::set_var("HOME", tmp.path());
        std::env::set_var("XDG_DATA_HOME", tmp.path().join("share_edit1"));
        let pool = init_pool(":memory:").await.unwrap();
        let case = create_case(
            &pool,
            NewCase {
                name: "e".into(),
                case_type: "诉讼".into(),
                source_folder: "/tmp/e".into(),
            },
        )
        .await
        .unwrap();
        let (doc_id, path) = seed_artifact(&pool, &case.id).await;
        let before = std::fs::read_to_string(&path).unwrap();
        let header_line = before.lines().next().unwrap().to_string();

        apply_edit(&pool, &doc_id, "五十万元", "八十万元", None, None)
            .await
            .map_err(|e| match e {
                ApplyErr::Retryable(m) | ApplyErr::Hard(m) => m,
            })
            .expect("应改成功");
        ...
        assert_eq!(
            ...
            header_line,
            "filing 头(含 ts)必须原样保留"
        );
        assert!(after.contains("八十万元") && !after.contains("五十万元"));
        // size_bytes 同步更新
        let size: i64 = sqlx::query_scalar("SELECT size_bytes FROM documents WHERE id = ?")
            ...
            .await
            .unwrap();
        assert_eq!(size as usize, after.len());
    }

    #[tokio::test]
    async fn apply_edit_empty_replace_deletes() {
        use crate::db::cases::{create_case, NewCase};
        use crate::db::init_pool;
        let _env_guard = crate::chat::HOME_ENV_LOCK.lock().await;
        let tmp = tempfile::tempdir().unwrap();
        std::env::set_var("HOME", tmp.path());
        std::env::set_var("XDG_DATA_HOME", tmp.path().join("share_edit2"));
        let pool = init_pool(":memory:").await.unwrap();
        let case = create_case(
            &pool,
            NewCase {
        ...
        .unwrap();
        let (doc_id, path) = seed_artifact(&pool, &case.id).await;
        apply_edit(&pool, &doc_id, "支付货款", "", None, None)
            .await
            .map_err(|e| match e {
                ApplyErr::Retryable(m) | ApplyErr::Hard(m) => m,
            })
        ...
        let after = std::fs::read_to_string(&path).unwrap();
        assert!(
            after.contains("判令被告五十万元"),
            "应删掉「支付货款」: {after}"
        );
    }

    #[tokio::test]
    async fn apply_edit_rejects_non_app_artifact() {
        use crate::db::cases::{create_case, NewCase};
        use crate::db::init_pool;
        let _env_guard = crate::chat::HOME_ENV_LOCK.lock().await;
        let tmp = tempfile::tempdir().unwrap();
        std::env::set_var("HOME", tmp.path());
        std::env::set_var("XDG_DATA_HOME", tmp.path().join("share_edit3"));
        let pool = init_pool(":memory:").await.unwrap();
        let case = create_case(
            &pool,
            NewCase {
        ...
        .await
        .unwrap();
        // 手插一行 source='scan' 的 AI 名文档(扫描进来的)→ 守卫必须拒改
        let doc_id = uuid::Uuid::new_v4().to_string();
        let now = chrono::Utc::now().to_rfc3339();
        sqlx::query(
            "INSERT INTO documents (id, case_id, source_path, filename, category, is_ai_artifact, \
             mime_type, size_bytes, modified_at, extraction_status, source, created_at) \
             VALUES (?, ?, '/tmp/scanned.md', 'scanned.md', '起诉状', 1, 'text/markdown', 10, ?, 'done', 'scan', ?)",
        )
        .bind(&doc_id)
        .bind(&case.id)
        .bind(&now)
        ...
        .await
        .unwrap();
        let err = apply_edit(&pool, &doc_id, "x", "y", None, None).await;
        assert!(
            matches!(err, Err(ApplyErr::Hard(_))),
            "source='scan' 的文件必须硬拒绝改写(防篡改原件)"
        );
    }

    #[tokio::test]
    async fn apply_edit_not_found_is_retryable() {
        use crate::db::cases::{create_case, NewCase};
        use crate::db::init_pool;
        let _env_guard = crate::chat::HOME_ENV_LOCK.lock().await;
        let tmp = tempfile::tempdir().unwrap();
        std::env::set_var("HOME", tmp.path());
        std::env::set_var("XDG_DATA_HOME", tmp.path().join("share_edit4"));
        let pool = init_pool(":memory:").await.unwrap();
        let case = create_case(
            &pool,
            NewCase {
        ...
        .await
        .unwrap();
        let (doc_id, _) = seed_artifact(&pool, &case.id).await;
        let err = apply_edit(&pool, &doc_id, "压根不存在的句子", "x", None, None).await;
        assert!(
            matches!(err, Err(ApplyErr::Retryable(_))),
            "找不到片段应是可重试(让模型换 find)"
        );
    }
}
